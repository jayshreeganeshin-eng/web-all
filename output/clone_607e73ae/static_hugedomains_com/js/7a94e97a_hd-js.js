				...
				if (contactHdv3Okay == 1) {
					...
					/* QQQ */
						...
						if (data == 'isValidFalse') {
							...
							$('#hdv3ContactFormSuccessDivID').show();
							...
							/* added 2025-03-12 for blank email notifications */
...
/* faq */
	...
	document.querySelectorAll('.faq-bi-det-summary').forEach(function(item){	
			/*
			item.addEventListener("click", function(){
				fab30DayLearnMoreFunc();
			});
			*/
			...
			item.addEventListener("toggle", (event) => {
			  if (item.open) {
				...
				/*	item.parentElement.style.border = "red solid 1px"; */
				/* alert('is open'); */
			  } else {
	...
	function toggleFaqDetSumv2(i) {


	  //dsObj = $(this).parents('.faq-section-block').find('.faq-bi-det-summary');
      
	  dsObj = $('#' + i);
      
	  
	  dsObj.slideDown("fast"); 

	  //dsObj.attr( "open", true ) ;  	//works  
	  //dsObj.attr('open', (dsObj.attr('open') == "false" ? true : false)); // no work
	  dsObja = dsObj.attr('open');
	 //alert(dsObja); // undefined or open  , not true and false
	  if (typeof dsObja === 'undefined')
		  dsObj.attr( "open", true ) ;
	  if (dsObja == 'open')
		  dsObj.attr( "open", false ) ;
	...
	$('.expand-thisx').on('click', function(e){
	  ...
	  /* faq-bi-det-title NO , faq-bi-det-summary yes*/
      //$(this).parents('.faq-section-block').find('.faq-bi-det-summary').slideDown("fast"); 
	  //$(this).parents('.faq-section-block').find('.faq-bi-det-summary').attr( "open", true ) ;  	  
    ...
    $('.expand-allx').on('click', function(e){
	  ...
	  dsObj = $(this).parents('.faq-page').find('.faq-bi-det-summary');
      dsObj.slideDown("fast"); 
