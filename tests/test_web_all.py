...
class TestAIEngine:
    ...
    def test_init_openrouter_requires_key(self):
        ...
        with pytest.raises(ValueError, match="API key"):
            AIEngine({"enabled": True, "provider": "openrouter", "api_key": None})
    ...
    async def test_summarize_content_disabled(self):
        ...
        engine = AIEngine({"enabled": False})
        result = await engine.summarize_content(
...
class TestServerAPI:
    ...
    def test_set_ai_config(self, client):
        ...
        config = {"enabled": True, "provider": "ollama", "base_url": "http://localhost:11434"}
    ...
    def test_download_job_not_completed(self, client):
        ...
        # First create a job
        clone_request = {"url": "https://example.com", "mode": "static"}
...
class TestCLI:
    ...
    def test_cli_help(self):
        ...
        result = subprocess.run(
            ["python", "-m", "web_all.cli", "--help"], capture_output=True, text=True
        )
    ...
    def test_cli_clone_help(self):
        ...
        result = subprocess.run(
            ["python", "-m", "web_all.cli", "clone", "--help"], capture_output=True, text=True
        )
...
class TestPerformance:
    ...
    async def test_concurrent_fetches(self, tmp_path):
        ...
        cloner = SiteCloner(output_dir=str(tmp_path), concurrency=10, depth=0)
        ...
        with patch.object(cloner.session, "get") as mock_get:
            ...
            tasks = [cloner.fetch_page(f"https://example.com/page{i}") for i in range(20)]
