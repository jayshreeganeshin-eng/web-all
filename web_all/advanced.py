                    ...
                    downloaded.append(
                        {
                            ...
                            "path": str(
                                self.output_dir
                                / f"{info.get('title', 'unknown')}.{info.get('ext', 'mp4')}"
                            ),
                        }
                    )
...
class FTPUploader:
    ...
    def _upload_recursive(
        self, ftp: ftplib.FTP, local_path: Path, remote_path: str, progress_callback=None
    ):
        ...
        if local_path.is_file():
            ...
            for item in local_path.iterdir():
                self._upload_recursive(ftp, item, f"{remote_path}/{dir_name}", progress_callback)
...
class MobileEmulator:
    ...
    async def capture_with_device(
        self, url: str, device: str = "iphone12", output_path: Optional[Path] = None
    ) -> str:
        """Capture page content emulating a specific mobile device."""
        if device not in self.MOBILE_DEVICES:
            raise ValueError(
                f"Unknown device: {device}. Available: {list(self.MOBILE_DEVICES.keys())}"
            )
...
class AuthManager:
    ...
    async def interactive_login(self, url: str, save_cookies: Optional[str] = None) -> List[Dict]:
        ...
        async with async_playwright() as p:
            ...
            if save_cookies:
                ...
                Path(save_cookies).write_text(json.dumps(self.cookies), encoding="utf-8")
    ...
    def load_cookies(self, cookie_file: str) -> List[Dict]:
        ...
        self.cookies = json.loads(Path(cookie_file).read_text(encoding="utf-8"))
        return self.cookies
