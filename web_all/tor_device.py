...
class TorManager:
    ...
    def detect_tor(self) -> Optional[int]:
        ...
        ports_to_check = [
            self.TOR_SOCKS_PORT_MAC,
            self.TOR_SOCKS_PORT_LINUX,
            self.TOR_SOCKS_PORT_WIN,
        ]
    ...
    def configure_for_onion(self, url: str) -> Dict[str, Any]:
        ...
        return {
            "proxy": {"server": self.proxy_server, "anonymize": True},
...
class DeviceOptimizer:
    ...
    async def responsive_capture(self, page: Page, url: str):
        ...
        await page.set_viewport_size(
            {"width": self.profile.viewport_width, "height": self.profile.viewport_height}
        )
...
async def create_tor_browser_session(
    playwright, tor_manager: TorManager, device_optimizer: DeviceOptimizer, headless: bool = True
):
    ...
    if tor_manager.is_onion_mode:
        context_options.update(
            tor_manager.configure_for_onion("")
        )  # Config already stored in manager
