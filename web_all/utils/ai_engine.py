...
class OpenRouterProvider(AIProvider):
    ...
    async def generate(self, prompt: str, system_prompt: str = "You are a helpful assistant.", model: str = "mistralai/mistral-7b-instruct:free") -> str:
        headers = {
...
class GroqProvider(AIProvider):
    ...
    async def generate(self, prompt: str, system_prompt: str = "You are a helpful assistant.", model: str = "llama3-8b-8192") -> str:
        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "Content-Type": "application/json",
        }
...
class HuggingFaceProvider(AIProvider):
    ...
    async def generate(self, prompt: str, system_prompt: str = "You are a helpful assistant.") -> str:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{self.base_url}/{self.model}", headers=headers, json=payload) as resp:
                if resp.status == 200:
                    data = await resp.json()
...
Return only comma-separated tags, no explanations."""
    ...
    async def filter_irrelevant_content(self, html_content: str) -> str:
        ...
        if not self.enabled or not self.provider:
            ...
            import re
            cleaned = re.sub(r'<nav[^>]*>.*?</nav>', '', html_content, flags=re.DOTALL | re.IGNORECASE)
...
{html_content[:3000]}"""
        ...
        try:
            return await self.provider.generate(prompt, system_prompt="You are an HTML cleaner. Return only cleaned HTML.")
        except Exception as e:
            print(f"AI filtering failed: {e}")
            return html_content
    
    async def analyze_and_enhance(self, html_content: str, url: str, output_dir: Path) -> Dict[str, Any]:
        ...
        try:
            results["summary"], results["structured_data"], results["tags"], results["cleaned_html"] = await asyncio.gather(*tasks, return_exceptions=True)
            
            # Handle exceptions in results
            for key, value in results.items():
                if isinstance(value, Exception):
                    print(f"AI task {key} failed: {value}")
                    results[key] = "" if key in ["summary", "cleaned_html"] else ([] if key == "tags" else {})
